...
class TestMatplotlibScatter:
    ...
    def test_scatter_xy_matches_synthetic(self, synthetic_ks4_output, synthetic_data):
        ...
        np.testing.assert_array_almost_equal(offsets[:, 1], synthetic_data["spike_depths"])
        plt.close(fig)
    ...
    def test_scatter_colors_for_all_scaling_modes(self, synthetic_ks4_output, scaling, n_bins):
        plotter = DriftPlotter(synthetic_ks4_output)
        fig = plotter.drift_map_plot_matplotlib(
            decimate=False, exclude_noise=False,
            amplitude_cmap_scaling=scaling, n_color_bins=n_bins,
        )
        ax = fig.axes[0]
        ...
        expected = processed.compute_amplitude_colors(scaling, n_bins, unit_normalise=True)
    ...
    def test_custom_point_size(self, synthetic_ks4_output):
        plotter = DriftPlotter(synthetic_ks4_output)
        fig = plotter.drift_map_plot_matplotlib(
            decimate=False, exclude_noise=False, point_size=42.0,
        )
        ax = fig.axes[0]
...
class TestMatplotlibHistogram:
    ...
    def test_histogram_counts_match_synthetic(self, synthetic_ks4_output, synthetic_data):
    ...
    def test_histogram_weighted_matches_synthetic(self, synthetic_ks4_output, synthetic_data):
...
def test_interactive_and_matplotlib_share_signature():
    ...
    """
    matplotlib_only_params = ["self", "add_histogram_plot",
