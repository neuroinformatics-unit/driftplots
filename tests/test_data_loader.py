...
class TestFromSorterOutput:
    ...
    def test_loaded_arrays_match_known_values_and_are_read_only(
        ...
        np.testing.assert_array_equal(
            loader._spike_times, synthetic_data["spike_times"]
        )
        np.testing.assert_array_almost_equal(
            loader._spike_depths, synthetic_data["spike_depths"]
        )
        np.testing.assert_array_equal(
            loader._spike_templates, synthetic_data["spike_templates"]
        )
        np.testing.assert_array_almost_equal(
            loader._spike_amplitudes, synthetic_data["spike_amplitudes"]
        )

        # Templates are loaded as-is from templates.npy (whitened for KS4)
        np.testing.assert_array_equal(
            loader.templates, synthetic_data["whitened_templates"]
        )

        # Channel locations should match exactly
        np.testing.assert_array_equal(
            loader.channel_locations, synthetic_data["channel_locations"]
        )
...
class TestProcessedData:
    ...
    def test_combined_decimate_and_filter(self, synthetic_ks4_output):
        ...
        np.testing.assert_array_equal(
            result.spike_times, only_filtered.spike_times[::2]
        )
        np.testing.assert_array_equal(
            result.spike_amplitudes, only_filtered.spike_amplitudes[::2]
        )
        np.testing.assert_array_equal(
            result.spike_depths, only_filtered.spike_depths[::2]
        )
...
class TestGoodUnitsOnly:
    ...
    def test_only_good_unit_spikes_kept(self, synthetic_ks4_output, synthetic_data):
        ...
        np.testing.assert_array_equal(
            result.spike_amplitudes, loader._spike_amplitudes[keep]
        )
