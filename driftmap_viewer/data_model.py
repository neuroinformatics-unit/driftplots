...
class DataModel:
    ...
    def get_template_heatmap(self, spike_index, view_mode):
        ...
        if np.any(np.logical_and(chan_x_spacings > COL_CUTOFF_UM, chan_x_spacings < 150)):
            warnings.warns(f"The spacings between x-locations: {chan_x_spacings} makes it difficult to distinguish"
    ...
    # TODO: CHECK THIS
    def compute_amplitude_colors(self, amplitude_scaling, n_color_bins, unit_normalise=False):
    ...
    def _compute_activity_histogram(
        ...
        bin_um = 2
        bins = np.arange(self.spike_depths.min() - bin_um, self.spike_depths.max() + bin_um, bin_um)
