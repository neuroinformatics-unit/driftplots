...
from driftmap_viewer.extractors import kilosort_helpers, kilosort_4, kilosort1_3  # TODO: remove underscorw?
...
def get_amplitudes(list_of_path_or_analyzer: list[Path | si.SortingAnalyzer], concatenate=False) -> np.ndarray:
    ...
    for path_or_analyzer in list_of_path_or_analyzer:
        ...
        if isinstance(path_or_analyzer, si.SortingAnalyzer):
            ...
            if ks_version == "kilosort4":
                ...
                spike_templates = np.load(path_ / "spike_templates.npy")  # rename spike_tempaltes_idx?
                templates = np.load(path_ / "templates.npy")      # rename unwhiten?

                spike_amplitudes = kilosort_4.compute_spike_amplitudes(templates, spike_templates, amplitudes)
            else:
